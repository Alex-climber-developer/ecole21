...
START_TEST(sin_test_1) {
  for (int R = 20; R >= -20; R -= 4) {
    for (double k = 2 * M_PI; k > -2 * M_PI; k -= M_PI / 17) {
      ck_assert_double_eq_tol(sin(k + R * M_PI), s21_sin(k + R * M_PI), 1e-6);
    }
  }
}
END_TEST
...
START_TEST(cos_test_1) {
  for (int R = 20; R >= -20; R -= 4) {
    for (double k = 2 * M_PI; k > -2 * M_PI; k -= M_PI / 17) {
      ck_assert_double_eq_tol(cos(k + R * M_PI), s21_cos(k + R * M_PI), 1e-6);
    }
  }
}
END_TEST
...
START_TEST(tan_test_1) {
  for (int R = 20; R >= -20; R -= 4) {
    for (double k = 2 * M_PI; k > -2 * M_PI; k -= M_PI / 9) {
      ck_assert_double_eq_tol(tan(k + R * M_PI), s21_tan(k + R * M_PI), 1e-6);
    }
  }
}
END_TEST
...
START_TEST(sin_cos_tan_test) {
  ...
  ck_assert_double_nan(s21_cos(-INFINITY));
  ck_assert_double_nan(s21_tan(-INFINITY));
  ck_assert_double_eq_tol(tan(M_PI / 2), s21_tan(M_PI / 2), 1e-7);
  ck_assert_double_eq_tol(tan(-M_PI / 2), s21_tan(-M_PI / 2), 1e-7);
}
END_TEST
